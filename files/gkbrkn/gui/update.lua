...
local setting_callbacks = {
    ...
    action = function( setting )
        ...
        if action_data ~= nil and ( deprecated == false or setting_get( FLAGS.ShowDeprecatedContent ) ) then
            ...
            if action_data then
                ...
                if is_action_unlocked( action_data ) == false then
                    hover_data = { GuiGetPreviousWidgetInfo( gui ) };
                end
            elseif setting_data.content.sprite then
                local left_click, right_click = GuiImageButton( gui, next_id(), 0, 0, "", setting_data.content.sprite );
                if left_click then
    ...
    perk = function( setting )
        ...
        if perk_data ~= nil then
            local perk_data = perk_data[setting_data.content.key];
            local toggle = false;
            ...
            if perk_data and perk_data.missing == true then
                GuiOptionsAddForNextWidget( gui, GUI_OPTION.DrawWaveAnimateOpacity );
            end
            GuiOptionsAddForNextWidget( gui, GUI_OPTION.ClickCancelsDoubleClick );
            local hover_data = nil;
            if perk_data then
                local left_click, right_click = GuiImageButton( gui, next_id(), 0, 0, "", perk_data.perk_icon );
                if left_click then
                    toggle = true;
                elseif right_click then
                    if setting_data.content.options.preview_callback ~= nil then
                ...
                if perk_data.missing == true then
                    hover_data = { GuiGetPreviousWidgetInfo( gui ) };
                end
            elseif setting_data.content.sprite then
                local left_click, right_click = GuiImageButton( gui, next_id(), 0, 0, "", setting_data.content.sprite );
...
for _,content_type in pairs( content_types ) do
    ...
    table.insert( OPTIONS, GKBRKN_CONFIG.register_option_tab_localized( content_type.id, GKBRKN_CONFIG.register_option_group( content_type.id, group_callbacks[content_type.id.."_page"], unpack( groups ) ) ) );
end
table.sort( content_type_selection, function( a, b ) return a.name < b.name end );

table.insert( OPTIONS, 1, GKBRKN_CONFIG.register_option_tab_localized( "gkbrkn_start_page", GKBRKN_CONFIG.register_option_group( "start_page", group_callbacks.start_page ) ) );
...
function do_gui()
    if setting_get( MISC.AutoHide.EnabledFlag ) == false or hidden == false then
        ...
        gokiui.parse_mod_settings( OPTIONS );
