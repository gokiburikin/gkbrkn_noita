...
local current_text = EntityGetVariableString( current_target, "gkbrkn_dps_tracker_text_highest", "" );
if current_target ~= 0 and last_text ~= current_text then
    last_text = current_text;
    local width,height = EntityGetFirstHitboxSize( current_target );
    local sprite = EntityGetFirstComponent( entity, "SpriteComponent", "gkbrkn_dps_tracker_highest" );
    if sprite then
