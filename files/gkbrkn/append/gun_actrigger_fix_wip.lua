...
function draw_actions( how_many, instant_reload_if_empty )
    ...
    if playing_permanent_card then
        GamePrint("permanent "..current_action.action_id)
    end
    ...
    if playing_permanent_card and (how_many == 1) then
        GamePrint("permanent action")
		local skipped_action = deck[1];
        if skipped_action ~= nil then
            GamePrint(skipped_action.id );
            table.remove( deck, 1 );
            table.insert( discarded, skipped_action );
        end
	end
end
